...
| Deux moteurs de scoring divergents | §4 | **Corrigé** (2026-07-16) — **convergence vers le moteur B**, décidée sur mesures : 89 % de divergence (71/80 titres), moteur A dégénéré (80/80 SURVEILLER sur données réelles ; 0/80 ACHETER même en simulation profonde), aucune notion de couverture ni de confiance. Coût jamais en cause (0.5 ms vs 8.7 ms pour 80 titres, contre 413 ms de `compute_metrics`) — ce qui **écarte aussi l'option pré-filtre** : un pré-filtre épargne un second étage coûteux, or il n'y en a pas. `score_opportunity` est désormais une projection de `horizon_strategy` ; `classify_label` utilise les seuils du CIO. **Divergence 89 % → 0 %.** |
| Composant `volume` du noyau B à 9.2/100 en moyenne | §4 | **Observé, non corrigé** (2026-07-16) — `clamp((anomalie − 1)/1.5 × 100)` donne 0 pour un volume normal (anomalie réelle moyenne : 0.70). Comportement **préexistant** du noyau, pilotant déjà les rapports du CIO ; le modifier serait un changement de scoring, pas d'architecture. Distribution réelle après convergence : 0 ACHETER / 29 SURVEILLER / 40 NEUTRE / 11 ÉVITER — saine et discriminante. |
| Seuil ACHETER (70) rarement atteint | §4 | **Observé, non corrigé** — score max observé 65.7 en simulation profonde. Décision produit, à trancher sur les données du moteur d'apprentissage une fois les prédictions mûres, pas à l'intuition. |
| Aucune authentification | §13 | Ouvert. |
| Aucune migration Alembic | §7, §10 | **Corrigé** (2026-07-16) — Alembic en place, baseline `50c59b463e1e` + migration `1b2587ed6aab` supprimant la table orpheline `signals`. `env.py` lit `settings.database_url` (jamais une URL dupliquée dans `alembic.ini`) et active `render_as_batch` sur SQLite. `init_db` estampille une base pré-Alembic sur la **baseline**, pas sur head — sinon la migration en attente serait marquée appliquée et l'orpheline survivrait. Upgrade/downgrade/re-upgrade **vérifiés sur copie de la vraie base** : données intactes. Migrations **jamais automatiques au démarrage** (`cli migrate`). 10 tests. |
| Table orpheline `signals` | §7 | **Corrigé** (2026-07-16) — supprimée par migration, avec `downgrade` qui recrée la structure. Les lignes ne sont pas restaurées : acceptable **ici uniquement**, car rien ne lisait cette table. |
| PostgreSQL en production | §11 | **Préparé et prouvé, non basculé** (2026-07-16) — `cli copy-database`, lecture seule sur la source, comptages vérifiés table par table, séquences Postgres réinitialisées. **Prouvé sur PostgreSQL 16.13 réel** : 60 034 lignes / 15 tables en 5.2 s, **13/13 endpoints OK**, écritures OK (`id=51` après un max de 50). Procédure complète dans [MIGRATIONS.md](MIGRATIONS.md). **Bascule non recommandée à ce stade** : aucun gain mesuré (1 045 ms vs ~990 ms sur la première requête), SQLite sert le marché en 4-34 ms derrière le cache, et il n'y a qu'un seul writer. |
...
| Couverture de tests | §12 | **Mesurée** — 68 % à l'audit → **78 %**. `api.py` 0→88 %, `notifications` 0→93 %, `views` 30→92 %, `company` 23→92 %, `fundamental` 17→89 %, `macro` 24→94 %. Restent faibles : `cli.py` (17 %), `scheduler.py` (38 %), `push.py` (29 %), `news.py` (36 %) ; à 0 % : `synthesis/claude.py` (LLM désactivé par défaut), `collectors/company.py`, `collectors/issuers.py`. |
| Analystes Phase 1b non testés avec données | §12 | **Corrigé** (2026-07-16) — 16 tests. Leurs tests n'exerçaient que la branche « données non collectées » ; le chemin qui lit un ratio, un actionnariat ou un taux directeur — celui qui tournera en production une fois les collectes hebdomadaires remplies — ne l'était jamais. |
| Dépendances runtime non épinglées | — | **Nouveau, corrigé** (2026-07-16) — `alembic` avait été livré sans être dans `requirements.txt` : le conteneur aurait échoué à l'import de `cli migrate`. `urllib3`, `cryptography` et `py_vapid` étaient importés directement tout en n'étant que transitifs. Garde-fou : `tests/test_packaging.py` analyse chaque import du paquet runtime. |
| `ARCHITECTURE_AI_ANALYST.md` affiche « DESIGN — no implementation » | §11 | **Corrigé** (2026-07-16) — le document d'architecture du dépôt était son document le moins fiable. En-tête refait, avec les deux points que la conception a eu faux. |
